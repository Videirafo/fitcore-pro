...
export function FitCoreRouteClient({ mode }: { mode: Mode }) {
  ...
  return (
    <section className="route-page" data-mode={mode}>
      ...
      {mode === "students" && <StudentsPanel role={role} data={students} detail={detail} state={state} error={error} onSubmit={submitStudent} onRefresh={() => loadRouteData("students")} />}
      {mode === "training" && <TrainingPanel role={role} students={students?.students || []} data={prescriptions} detail={detail} state={state} error={error} firstStudentId={firstStudentId} onSubmit={submitPrescription} onReview={reviewPrescription} onRefresh={() => loadRouteData("training")} />}
...
function RoleDashboard({ session, navigation, team, students, prescriptions, executions, evolution }: { session: Json | null; navigation: Json | null; team: Json; students: Json; prescriptions: Json; executions: Json; evolution: Json }) {
  ...
  const counts = {
    users: Number(team?.total_users || team?.users?.length || 0),
    students: Number(students?.total || students?.students?.length || evolution?.summary?.students || 0),
    workouts: Number(prescriptions?.total || prescriptions?.prescriptions?.length || prescriptions?.workouts?.length || 0),
    executions: Number(executions?.total || executions?.executions?.length || evolution?.summary?.executions || 0),
  };
  const profile = role === "gestor"
    ? { title: "Dashboard executivo", text: "Visão completa da operação: equipe, alunos, treinos, execuções e evolução.", kpis: [["Equipe", counts.users], ["Alunos", counts.students], ["Treinos", counts.workouts], ["Execuções", counts.executions]], actions: [["/equipe", "Gerenciar equipe"], ["/alunos", "Cadastrar aluno"], ["/treinos", "Prescrever treino"], ["/evolucao", "Ver evolução"]] }
    : role === "professor"
      ? { title: "Painel do professor", text: "Priorize alunos ativos, treinos pendentes e evolução de quem está sob sua responsabilidade.", kpis: [["Alunos", counts.students], ["Treinos", counts.workouts], ["Execuções", counts.executions], ["Pendências", Math.max(0, counts.workouts)]], actions: [["/alunos", "Meus alunos"], ["/treinos", "Treinos pendentes"], ["/execucao", "Acompanhar execução"], ["/evolucao", "Progresso"]] }
      : role === "aluno"
        ? { title: "Painel do aluno", text: "Acesse o treino liberado, registre a execução e acompanhe sua evolução.", kpis: [["Treinos", counts.workouts], ["Execuções", counts.executions], ["Evolução", evolution?.student?.progress_percent ?? 0], ["Esforço", evolution?.student?.average_effort ?? 0]], actions: [["/treinos", "Meu treino"], ["/execucao", "Executar treino"], ["/evolucao", "Minha evolução"]] }
        : { title: "Entre para continuar", text: "Crie sua operação ou acesse o painel com uma credencial real.", kpis: [["Negócio", "Novo"], ["Acesso", "Seguro"], ["Equipe", "Papéis"], ["Dados", "Reais"]], actions: [["/onboarding", "Criar negócio"], ["/login", "Entrar"]] };
  return (
    <section className={`role-dashboard role-dashboard-${role}`}>
      <div className="role-copy"><span>{role === "visitante" ? "Primeiro acesso" : "Visão por papel"}</span><strong>{profile.title}</strong><p>{profile.text}</p></div>
      <div className="role-metrics">{profile.kpis.map(([label, value]) => <Metric key={String(label)} label={String(label)} value={value} />)}</div>
      <div className="role-actions">{profile.actions.map(([href, label]) => <Link key={String(href)} className="primary-pill" href={String(href)}>{label}</Link>)}</div>
      {navigation?.menu?.length ? <small className="role-note">{navigation.menu.length} áreas disponíveis para esta sessão</small> : null}
    </section>
  );
}
...
function HomePanel({ session }: { session: Json | null }) {
  const role = String(session?.actor_role || "visitante").toLowerCase();
  const modules = [
    ["/onboarding", "Criar negócio", "Abra uma unidade fitness com gestor proprietário e equipe inicial.", ["visitante"]],
    ["/login", "Entrar", "Acesse seu painel com senha ou código de acesso.", ["visitante"]],
    ["/equipe", "Equipe", "Crie usuários, defina papéis e mantenha acessos organizados.", ["gestor"]],
    ["/alunos", "Alunos", "Cadastre alunos, objetivos, nível e frequência semanal.", ["gestor", "professor"]],
    ["/treinos", "Treinos", "Prescreva, revise e acompanhe treinos liberados.", ["gestor", "professor", "aluno"]],
    ["/execucao", "Execução", "Registre ou acompanhe treinos em andamento.", ["gestor", "professor", "aluno"]],
    ["/evolucao", "Evolução", "Veja histórico, frequência e progresso por aluno.", ["gestor", "professor", "aluno"]],
  ];
  const visible = modules.filter(([, , , roles]) => (roles as string[]).includes(role));
  return <div className="module-grid">{visible.map(([href, title, text]) => <Link className="module-card" href={String(href)} key={String(href)}><small>{role === "visitante" ? "Comece" : "Operação"}</small><strong>{title}</strong><span>{text}</span></Link>)}</div>;
}
...
function TeamPanel({ role, data, detail, state, error, onCreate, onInvite, onRefresh }: { role: string; data: Json; detail: Json | null; state: LoadState; error: string; onCreate: (event: FormEvent<HTMLFormElement>) => void; onInvite: (event: FormEvent<HTMLFormElement>) => void; onRefresh: () => void }) {
  ...
  if (role !== "gestor") return <PermissionPanel title="Equipe" text="Somente gestor administra usuários, convites e papéis da unidade." />;
