...
type Session = { actor_role?: string } | null;
...
export function FitCoreHeaderActionsClient() {
  ...
  if (role === "visitante") {
    return <div className="header-actions"><Link href="/login">Entrar</Link><Link className="primary-pill topbar-cta" href="/onboarding">Criar negócio</Link></div>;
  }
  if (role === "aluno") {
    return <div className="header-actions"><Link href="/execucao">Treino do dia</Link><Link className="primary-pill topbar-cta" href="/evolucao">Minha evolução</Link></div>;
  }
  if (role === "professor") {
