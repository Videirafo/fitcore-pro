...
const features: Array<[string, string, IconName, string]> = [
  ["Criar negócio", "Configure sua estrutura e comece hoje.", "chart", "/onboarding"],
  ["Equipe", "Gerencie sua equipe e permissões.", "users", "/equipe"],
  ["Alunos", "Cadastre, organize e acompanhe.", "user", "/alunos"],
  ["Treinos", "Prescreva treinos com agilidade.", "dumbbell", "/treinos"],
  ["Execução", "Acompanhe execução e engajamento.", "play", "/execucao"],
  ["Evolução", "Veja resultados com dados reais.", "chart", "/evolucao"],
];
...
const sidebar = [["Visão geral", "home", "/dashboard"], ["Dashboard", "grid", "/dashboard"], ["Onboarding", "spark", "/onboarding"], ["Equipe", "users", "/equipe"], ["Professores", "users", "/equipe"], ["Alunos", "users", "/alunos"], ["Biblioteca de exercícios", "grid", "/biblioteca"], ["Treinos", "dumbbell", "/treinos"], ["Prescrição", "clipboard", "/treinos"], ["Execução", "play", "/execucao"], ["Evolução", "chart", "/evolucao"], ["Segurança & LGPD", "shield", "/seguranca"], ["Configurações", "gear", "/configuracoes"]] as const;
...
export function FitCoreExactDashboard() {
  return <main className="fcx-console">
    ...
    <section className="fcx-main"><header className="fcx-top"><Link href="/configuracoes" className="fcx-unit"><Icon name="building"/><strong>Academia Movimento</strong><small>Unidade Matriz</small></Link><form action="/biblioteca"><Icon name="search"/><input name="q" placeholder="Buscar alunos, treinos, exercícios ou recursos..."/><kbd>⌘ K</kbd></form><div><Link className="fcx-bell" href="/auditoria"><Icon name="bell"/><b>3</b></Link><Link className="fcx-user" href="/configuracoes"><i>FL</i><strong>Fernando Lima<small>Gestor</small></strong></Link><em>Gestor</em></div></header>
      <section className="fcx-welcome"><div><h1>Olá, Fernando! 👋</h1><p>Aqui está o panorama da sua academia hoje.</p></div><div className="fcx-tabs"><Link className="active" href="/dashboard"><Icon name="chart"/>Gestor</Link><Link href="/treinos"><Icon name="users"/>Professor</Link><Link href="/execucao"><Icon name="user"/>Aluno</Link></div><aside><span>Terça-feira, 9 de setembro de 2025</span><q>Mais que treinos, mais pessoas saudáveis.</q></aside></section>
...
function FitCoreExactAgents() { return <article className="fcx-panel fcx-agents"><header><h2><Icon name="spark"/>Agentes IA <span>BETA</span></h2><Link href="/agents">Ver todos →</Link></header><div className="fcx-agent-layout"><ul>{["Gerar plano de treino", "Analisar evolução de alunos", "Sugerir ajustes de carga", "Revisar execução de exercícios", "Tirar dúvidas instantâneas"].map(x => <li key={x}><Link href="/agents"><b>{x}</b><span>Recomendações baseadas em dados</span><i>›</i></Link></li>)}</ul><aside><div className="fcx-bot"><i/><b>FC</b></div><strong>Olá, Fernando!</strong><p>Como posso te ajudar hoje?</p><Link href="/agents">Conversar com a IA →</Link></aside></div></article>; }
