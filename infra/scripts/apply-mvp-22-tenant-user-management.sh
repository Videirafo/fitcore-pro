...
psql "$FITCORE_DATABASE_URL" -v ON_ERROR_STOP=1 -f infra/sql/008-mvp-22-tenant-user-management.sql
mkdir -p /etc/systemd/system/fitcore-api.service.d
cat > /etc/systemd/system/fitcore-api.service.d/50-fitcore-tenant-users.conf <<'EOF'
[Service]
Environment="FITCORE_TENANT_USER_MANAGEMENT_ENABLED=true"
EOF
...
echo "MVP-22 aplicado: gestão real de usuários por tenant ativa."
