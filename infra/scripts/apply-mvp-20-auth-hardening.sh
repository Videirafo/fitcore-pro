...
psql "$FITCORE_DATABASE_URL" -v ON_ERROR_STOP=1 -f infra/sql/006-mvp-20-auth-hardening.sql
bash infra/scripts/seed-mvp-20-admin-credential.sh
mkdir -p /etc/systemd/system/fitcore-api.service.d
cat > /etc/systemd/system/fitcore-api.service.d/40-fitcore-auth-hardening.conf <<'EOF'
[Service]
Environment="FITCORE_ENV=production"
Environment="FITCORE_AUTH_HARDENING_ENABLED=true"
Environment="FITCORE_DEMO_LOGIN_ENABLED=false"
Environment="FITCORE_CREDENTIAL_LOGIN_ENABLED=true"
Environment="FITCORE_AUTH_RATE_LIMIT_MAX_ATTEMPTS=5"
Environment="FITCORE_AUTH_RATE_LIMIT_WINDOW_SECONDS=300"
Environment="FITCORE_AUTH_RATE_LIMIT_BLOCK_SECONDS=900"
Environment="FITCORE_SESSION_TTL_SECONDS=28800"
Environment="FITCORE_PUBLIC_URL=https://fitcore.marcaia.app"
EOF
...
echo "MVP-20 aplicado: hardening ativo, demo bloqueado, credencial real e logs por tenant."
