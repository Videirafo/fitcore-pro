...
const css = readFileSync("apps/site/app/globals.css", "utf8") + "\n" + readFileSync("apps/site/app/premium-console.css", "utf8");
const server = readFileSync("services/api/server.mjs", "utf8");
for (const token of ["AgentPanel", "ExerciseMediaGrid", "LibraryPanel", "SecurityPanel", "AuditPanel", "/api/mvp-32/agent", "ExerciseMotionVisual", "Sair com segurança"]) check(client.includes(token) || header.includes(token) || nav.includes(token), `token ausente no Next: ${token}`);
...
console.log("OK: MVP-32 workspace premium, agents, orientação visual e navegação completa validados por contrato.");
